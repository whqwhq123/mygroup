<template>
  <div class="model">
    <el-row>
      <el-col :span="6" class="colleft">
        <el-row type="flex" class="row-bg leftTit" justify="space-between">
          <el-col :span="8">
            <div class="leftTit_text">基本信息</div>
          </el-col>
          <el-col :span="8">
            <div class="leftTit_tomod">
              <el-button type="text" size="mini" style="font-size: 14px; color: #3b86ff" v-if="get_role_function('100400120')" @click.stop="() => append()">
                +添加品牌
              </el-button>
            </div>
          </el-col>
      ...
      <el-scrollbar class="boxscroll">  
              <el-tree :data="treedata" @node-click="nodeclickFun" node-key="id">
                <div class="custom-tree-node treecom" :style="data.type == 1 ? 'background: #E4E6ED;border-bottom: 1px solid #D7DAE2': ''"
                  ...
                  <div class="node_titright">
                    <el-button type="text" size="mini" v-if="data.type!=2 && get_role_function('100400120') "  style="color: #3b86ff" @click.stop="() => append(node, data)">
                      +添加子品牌
                    </el-button>
                    <el-button type="text" v-if="get_role_function('100400130')" size="mini" icon="el-icon-delete" :style="data.type != 1 ? 'color: #929496;' : ''"
      ...
      <el-col :span="18">
        <el-row class="row-bg rightTit">
          <el-col>
            <div class="rightTit_text">车系管理</div>
          </el-col>
        </el-row>
        ...
        <template>
          <el-row class="row-bg rowPid">
            <el-col :span="6">
              <el-input placeholder="请输入内容" v-model="inputVal" @keyup.enter.native="entterFun" clearable>
              </el-input>
            </el-col>
            <el-col :span="3">
              <el-button size="small" @click.native="entterFun" style="
                color: #ffffff;
                background: #43425d;
                border-radius: 4px;
                width: 70px;
          ...
          <el-row class="row-bg rowPid">
            <el-col>
              <div class="rightTit_text">
                <model-tabs-com :tableData="tableData" :carseriesdefault="carseriesdefault"  @tabruneve="tabruneveFun" />
                <!-- 分页 -->
    ...
    <!-- 添加子品牌 -->
    <el-dialog title="添加子品牌" append-to-body width="25%" :visible.sync="dialogSubBrand">
      <el-form ref="form" :model="dialogSubData" label-width="110px" size="mini">
        <el-form-item label="父级品牌">
          <el-input v-model="dialogSubData.subbrandTit" :disabled="true"></el-input>
        </el-form-item>
        <el-form-item label="子品牌名称">
          <el-select v-model="dialogSubData.subbrandsel" placeholder="请选择子品牌名称">
    ...
    <!-- 添加车系 -->
    <el-dialog title="添加车系" append-to-body width="25%" :visible.sync="dialogAddmodel">
      <el-form ref="form" :rules="rules" :model="dialogSubseries" label-width="60px" size="mini">
        <el-form-item label="车系" prop="subbrandTit">
          <el-input v-model="dialogSubseries.subbrandTit"></el-input>
        </el-form-item>
        <el-form-item label="车种" prop="subbrandsel">
          <el-select v-model="dialogSubseries.subbrandsel" placeholder="请选择子品牌名称">
            <!-- vehicle -->
    ...
    <!-- 添加品牌 -->
    <!-- modeladdBrandCom -->
    <modeladd-brand-com v-if="isbrand" :isbrand="isbrand" :isbrandFun.sync="isbrand" />
    <!-- 车型管理和限售管理 -->
    <limit-modelrun-com v-if="islimitModel" :limitModeldata="limitModeldata" :islimitModel="islimitModel" :treeF="treeF"
      :treeS="treeS" :islimitModelFun.sync="islimitModel" />
  </div>
</template>
...
<script>
  ...
  import {
    ...
    getCarGroupByMakeId,
    addCarMake,
    addCustomizeCarGroup,
    getCarModelList,
    getLevelNames,
    addCustomizeCarModel,
    deleteCarMakeOrCarGroup
  } from "api/index.js";
    ...
    methods: {
      ...
      append(node, data) {
        if (data) {
          ...
          this.dialogSubData.subbrandcus = ''
      ...
      submitForm(i) {
        ...
        if (i == 1) {
          ...
          else if (this.dialogSubData.subbrandsel != '') {
            this.addCarMakeFun()
          }
          //添加自定义品牌
          else  if(this.dialogSubData.subbrandcus != '') {
            this.addCustomizeCarGroupFun()
          }
      ...
      remove(node) {
        this.$confirm("确认删除?", "提示", {
          ...
          deleteCarMakeOrCarGroup(data).then(res => {
            if (res.code == 0) {
              this.$notify({
                title: "成功",
                message: "删除成功",
      ...
      async getCarGroupByMakeIdFun(makeId) {
        let res = await getCarGroupByMakeId({
          makeId
        })
        if (res.code == 0) {
      ...
      async addCarMakeFun() {
        let res = await addCarMake({
          makeIds: this.dialogSubData.subbrandid,
          groupIds: this.dialogSubData.subbrandsel
        })
        if (res.code == 0) {
        //  console.log(res)
          this.$message({
      ...
      async addCustomizeCarGroupFun() {
        let res = await addCustomizeCarGroup({
          makeId: this.dialogSubData.subbrandid,
          groupName: this.dialogSubData.subbrandcus
        })
        if (res.code == 0) {
          this.$message({
            message: '添加成功',
